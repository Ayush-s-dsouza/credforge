...
### D-077: DISCOVER_SIGNUP is now reachable from the web -- live generation for any vendor with no recipe at all

**The gap this closes:** all three verification questions the user asked
(deploy freshness, web reachability, Alpha Vantage precedence) mapped to
real, separate gaps -- D-075 closed the third (generated recipes now win,
and survive a redeploy), this entry closes the second: `webapp/main.py`'s
`/api/run` had no path that ever called `discover_signup` at all. Every
prior live generation this session ran from an ad-hoc script; the site
itself could only ever replay an ALREADY-known recipe.

**`/api/run` now has three routes, not two, chosen before the pipeline
ever runs:** (1) a hand-written recipe matches (`_match_recipe_identity`,
unchanged) -> the existing live-replay path, byte-for-byte unchanged. (2)
no hand-written match, but a PREVIOUSLY GENERATED recipe matches this
exact identity_key (`_match_generated_recipe`, new -- re-reads
`load_generated_recipes(settings.data_dir)` fresh every request, since a
recipe a prior request generated in this same process has to be visible
to the very next one) -> also routes through the existing live-replay
path, now genuinely exercising "does a second run replay the generated
recipe with zero LLM calls" as a real, repeatable web-request behavior,
not just a scratch-script claim. (3) no recipe of any kind matches, and
live provisioning is enabled -> `discover_signup(app_name, ...,
live=True, explain=sink)` runs INSTEAD of `run_app`, streaming through
the exact same SSE channel via D-076's instrumentation. Its own internal
gating (GATE=AUTO, signup-form-to-key archetype, idempotency guard, every
hard stop) decides whether a real signup is ever actually attempted --
this entry adds routing, not new judgment calls; DISCOVER_SIGNUP's
existing safety logic is completely untouched, per the user's explicit
instruction never to bypass a hard stop to make a demo work. Verified
live and locally: a real end-to-end run against GitHub (real
RESOLVE/DISCOVER/CLASSIFY/GATE, real GATE=AUTO) correctly stopped at
"wrong archetype" (OAuth, not signup-form-to-key) with zero browser
action taken -- exactly the safe, no-signup-attempted outcome the routing
is supposed to produce for the common case.

**A freshly generated recipe is registered into the running process's own
driver, not just written to disk.** `providers_live` (webapp/main.py) is
built once, at process startup, from whatever `load_generated_recipes`
found at that moment -- a recipe generated by request N would be invisible
to request N+1's `PlaywrightBrowserDriver` without this. A new public
method, `PlaywrightBrowserDriver.register_recipe(domain, recipe)`,
lets the request handler inject it immediately after a successful
generation. Best-effort: if this fails for any reason, the recipe is
still safely on disk, and the very next request would fall through to
route (2) above and find it there regardless -- a missed fast path, not a
correctness gap.

**Guards, each mapped to something the user explicitly required:**
- *Its own cap, lower than `LIVE_RUN_CAP`* -- `GENERATION_CAP` (default
  10, `/api/status`), reserved BEFORE calling `discover_signup`,
  unconditionally, whenever no existing recipe matched -- a true upper
  bound on live-generation attempts even though many of them (wrong
  archetype, GATE not AUTO) never reach a real browser at all. Shown as
  its own decrementing badge on the page, same pattern as the existing
  live-run badge.
- *Per-IP limit* -- the existing `_check_rate_limit` already applies to
  every `/api/run` call uniformly; no new mechanism needed, no gap to
  close.
- *Every hard stop intact* -- structural (CAPTCHA/payment field
  detection, D-065), the LLM's own blockers judgment, the confidence
  threshold, SSO-only, phone verification: none of this is touched by
  this entry. `discover_signup` is called exactly as the CLI or any
  scratch script would call it.
- *Idempotency guard before any browser action* -- already `discover_signup`'s
  own first check (`registry.find_open_provision`), unchanged.
- *Fresh per-run email alias* -- already `discover_signup`'s own
  `providers.email.alias_for(identity_key, suffix=run_id[-8:])`, unchanged.
- *Generated recipes persist to a committed path, not an ephemeral run
  dir* -- true for a vendor a human has already reviewed and committed
  (D-075's Alpha Vantage example); NOT achievable for a vendor Stage 3
  generates live for the first time -- nobody has reviewed it yet, so
  committing it to git automatically would mean trusting an unreviewed
  LLM-driven signup's output as if it were hand-verified. `settings.data_dir`
  (survives within one deployment's process lifetime, enough for the
  "second run replays" claim) plus the runtime `register_recipe` injection
  above is the honest, only-possible answer for a genuinely first-time
  generation; a human reviewing and promoting a good one into
  `examples/generated_recipes/` afterward (exactly what happened for
  Alpha Vantage) remains a manual step, stated here plainly rather than
  implied to be automated.
- *UI states plainly before the run* -- new copy in the "Research +
  gating -- try any app" section states this in the page itself, not just
  in the SSE mode banner (which also changed: a new "LIVE GENERATION"
  message, distinct from both "RESEARCH + GATING" and "LIVE CREDENTIAL
  ACQUISITION", explicit that this is not a simulation).

**Verified locally, frontend included -- not just the backend SSE
stream.** No browser GUI is available in this environment to inspect
visually, so a Playwright script drove the actual local page: typed
"GitHub", clicked Run, and asserted on the real rendered DOM -- the new
`badge-generation-runs` badge showing the cap decrementing, the full
stage-by-stage log rendering with correct HTML escaping, the
`generation_done` event's distinct rendering path (`DiscoverSignupResult`
has a different shape than `HandoffArtifact` -- field_map/recipe_path/
credential_vault_ref, not api/credential/tos -- so it's rendered by its

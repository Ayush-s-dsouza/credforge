...
from ..pipeline.llm_call_log import NULL_LLM_CALL_LOG, LlmCallLog
...
def _raw_text(response) -> str:
    """The literal text the model emitted, before Pydantic parsing --
    `output_format` still returns ordinary text content blocks under the
    hood (D-078's audit log wants what the model actually said, not a
    re-serialization of the parsed object, even though the two are
    supposed to be equivalent for a well-formed response)."""
...
class AnthropicSignupFormAnalyzer:
    def __init__(self, *, api_key: str, call_log: LlmCallLog = NULL_LLM_CALL_LOG) -> None:
    ...
    async def identify_reveal_trigger(
        ...
        prompt = (
            ...
            "only render after a specific element is clicked -- an element whose apparent "
            "purpose is to open, reveal, or navigate to a registration or credential-request "
            "form, rather than appearing on a timer. Below is every visible, clickable "
            "link/button on the page, one JSON object per line: its selector, tag, visible "
            "text, and href (if any).\n\n"

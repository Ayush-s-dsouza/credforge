            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        "more features (e.g. NASA's api.nasa.gov: no key needed to "
                        "explore the data, but a free developer key raises the rate "
                        "limit substantially) -- classify auth_scheme as that "
                        "credential's real scheme (e.g. api_key) in this case, never "
                        "\"none\". Reserve auth_scheme=\"none\" exclusively for an API "
                        "that has no credential mechanism to acquire at all. "
                        "Separately, report auth_required: \"required\" (a credential "
                        "is mandatory for any access), \"optional\" (the API works "
                        "without one, but a real, acquirable credential also exists), "
                        "or \"none\" (matches auth_scheme=\"none\" -- no credential "
                        "mechanism exists). A prior discovery pass found: "
